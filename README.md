...
1.  **Root access** (required to manage network interfaces).
2.  **nftables**: Used for backend firewall rules (`apt install nftables`).
3.  A running WireGuard/AmneziaWG interface (Kernel or Userspace).
...
### 2. Initialize Server
Run `jwg` for the first time. It will auto-detect your Public IP, find your NAT interface, and apply necessary firewall rules.
